...
3. `enctype`: 在 POST 方法下，对表单内容在请求包体中的编码方式
   ...
   - `multipart/form-data`
     ...
     - `last boundary`结尾
   - `application/x-www-form-urlencoded`: 通常用于提交表单数据，例如登录表单、搜索表单等, 适用于提交简单的表单数据
   - `multipart/form-data`: 是一种较为复杂的编码方式，它可以用于提交文件等二进制数据，在这种编码方式下，请求体中的数据被分成多个部分，每个部分都有自己的头信息和正文内容，并使用一个唯一的分隔符将它们分开。这种编码方式通常用于上传文件等二进制数据
...
2. IP：根据 IP 地址穿越网络传达数据
3. 源主机：应用层->传输层(tcp)->网络层(ip)->数据链路层->物理层
4. 路由器：物理层->数据链路层->网络层(ip)->数据链路层->路由器：物理层
5. 交换机：物理层->数据链路层->交换机：物理层
6. 源主机：物理层->数据链路层->网络层(ip)->传输层(tcp)->应用层
7. tcp 的作用：在 ip 协议之上，解决网络通讯可依赖的问题
   - 点对点（不能广播、多播），面向连接
   - 双向传递（全双工）
   - 字节流：打包成报文段，保证有序接收，重复报文自动丢弃
     - 缺点：不维护应用报文的边界（对比 HTTP GRPC），需自己定义/n 来结尾
...
5. 应对 SYN 攻击
   ...
   4. `TCP_DEFER_ACCEPT` - 服务器收到 tcp 连接时不处理，放到队列中，等收到报文时再处理，使应用程序效率更高
6. tcp 会把数据拆分成很多个报文段
   - 流分段的依据：
     - MSS：防止 IP 层分段
     - 流控：接收端的能力
   - MSS：Max Segment Size
     ...
     - MSS 选择目的
       - 尽量么个 Segment 报文段携带更多的数据，以减少头部空间占用比率
       - 防止 Segment 被某个设备的 IP 层基于 MTU 拆分
     - MSS 分类
       - 发送方最大报文段 SMSS：ＳＥＮＤＥＲ　ＭＡＸＩＭＵＭ　ＳＥＧＭＥＮＴ　ＳＩＺＥ
       - 接收方最大报文段ＲＭＳＳ： ＲＥＣＥＩＶＥ　ＭＡＸＩＭＵＭ　ＳＥＧＭＥＮＴ　ＳＩＺＥ

## 38.重发与确认

1. 定时器：每次发送报文的时候，启动 1 个定时器，如果时间内没有收到服务器回传的确认报文，则定时器任务重发报文
2. 每一条消息发送带上标识，就能确保是哪一条报文没有接收到，接收到报文再发送会重启 1 个定时器
3. 定时器的时间设置应该略大于 RTT 时间

## 39.滑动窗口

1. 发送数据的时候会占用可用窗口的大小，接收到 ack 后会恢复可用窗口的大小
2. 滑动窗口存在操作系统的缓存区

## 40.减少小报文提高网络效率

- 每个 segment 的报文中，会有固定 20 个字节的 TCP 头部，固定 20 个字节的 IP 头部

## 41.拥塞控制

1. 慢启动
   - 拥塞窗口：通告窗口（接收窗口）和发送窗口（数据窗口和 ACK）
   - 开始只定义 1 个拥塞窗口，每接收到 1 个 ACK，拥塞窗口扩充一倍，指数级增长
2. 拥塞避免
   - 慢启动阈值：到达一定的阈值后，慢启动不再使用指数级增长，采用线性方式增长
   - 每次发生丢包，就重新走慢启动和设置阈值更小
3. 快速重传
   - 接收方收到失序数据段后，发送所期待的缺口 ACK 序列号，接收到填充数据后，再发出下一个 ACK 序列号
   - 发送方接收到 3 个重复的失序 ACK 段，不再等待定时器，基于快速重传的机制，重发报文段
4. 快速恢复
   - 启动快速重传且正常未失序 ACK 段到达前，启动快速恢复

## 42.多路复用

1. 在一个信道上传输多路信号或数据流的过程和技术
2. 非阻塞 socket: 同时处理多个 TCP 连接
3. epoll 高效处理连接
   - 活跃连接只在总链接的一小部分

## 43.IPv4

1. 一个 IPv4 地址是由 32 位的二进制数组成
2. IPv4 的地址空间：2^32 个
3. IP 地址是由 IANA 分配的，根据地区层层分配下来
