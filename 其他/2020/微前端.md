...
6. `DSL`模板引擎
    ...
    ```
7. 双向绑定
    1. 手动绑定，通过手动`set`和`get`数据来触发UI或数据变化
    2. 脏检查，发生指定的事件（如`http`请求，`dom`事件），遍历数据相应的元素，然后进行数据比较，对变化的数据进行操作
    3. 数据劫持，通过hack的方式（`Object.defineProperty()`）对数据的`setter`和`getter`进行劫持，在数据变化时，通知相应的数据订阅者，以触发相应的监听回调
    ```html
...
8. 前端路由
    ...
    2. `Hash`
        ...
        * 浏览器访问网页，网页中URL中带有`hash`，页面被定位到id(或name)与Hash值一样的元素的位置
    3. 自造`Hash`路由管理器
        ```js
        function Router() {
            this.routes = {};
...
5. 服务端渲染
    ...
    2. 渲染的方式
        ...
        * 预渲染，用程序生成带真实数据的html， 放到静态站点服务器上
## 7.组件化架构
1. 字体
